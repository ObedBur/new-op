...
const createMockUserValidationService = () => ({
    ...
    getInitialKycStatus: jest.fn().mockReturnValue(client_1.KycStatus.NOT_REQUIRED),
...
const createMockVendor = (overrides = {}) => createMockUser({
    id: 'vendor-id-123',
...
const createRegisterDto = (role = 'CLIENT') => {
    const base = {
...
describe('AuthService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                { provide: token_service_1.TokenService, useValue: tokenService },
                { provide: otp_service_1.OtpService, useValue: otpService },
                { provide: password_service_1.PasswordService, useValue: passwordService },
                { provide: user_validation_service_1.UserValidationService, useValue: userValidationService },
            ],
        }).compile();
    ...
    describe('register', () => {
        ...
        it('should successfully register a new VENDOR with boutiqueName', async () => {
            ...
            userValidationService.getInitialKycStatus.mockReturnValue(client_1.KycStatus.PENDING);
            prisma.user.findFirst.mockResolvedValue(null);
            prisma.user.create.mockResolvedValue(createMockVendor({ email: dto.email, isVerified: false, kycStatus: client_1.KycStatus.PENDING }));
            const result = await service.register(dto);
            expect(result).toEqual({
                success: true,
                message: 'Registration successful. Verify OTP.',
        ...
        it('should reject VENDOR registration without boutiqueName', async () => {
            const dto = { ...createRegisterDto('VENDOR'), boutiqueName: undefined };
            prisma.user.findFirst.mockResolvedValue(null);
            await expect(service.register(dto)).rejects.toThrow(new common_1.HttpException('Boutique name is required for vendors', common_1.HttpStatus.BAD_REQUEST));
            expect(prisma.user.create).not.toHaveBeenCalled();
        });
        it('should reject duplicate email', async () => {
            const dto = createRegisterDto('CLIENT');
            prisma.user.findFirst.mockResolvedValue(createMockUser());
            await expect(service.register(dto)).rejects.toThrow(new common_1.HttpException('User already exists', common_1.HttpStatus.CONFLICT));
            expect(prisma.user.create).not.toHaveBeenCalled();
        });
    ...
    describe('verifyOtp', () => {
        ...
        it('should throw error if user not found during verification', async () => {
            ...
            await expect(service.verifyOtp({ email: 'nonexistent@example.com', otp: '123456' }))
                .rejects.toThrow(new common_1.HttpException('User not found', common_1.HttpStatus.NOT_FOUND));
        });
    });
    describe('login', () => {
        ...
        it('should successfully login an approved VENDOR', async () => {
            ...
            expect(result.user.role).toBe(client_1.UserRole.VENDOR);
        });
        ...
        it('should reject wrong password', async () => {
            ...
            await expect(service.login({ email: user.email, password: 'wrongpassword' }))
                .rejects.toThrow(new common_1.HttpException('Invalid password', common_1.HttpStatus.UNAUTHORIZED));
        });
        it('should reject unverified user', async () => {
            ...
            await expect(service.login({ email: user.email, password }))
                .rejects.toThrow(new common_1.HttpException('Account not verified', common_1.HttpStatus.FORBIDDEN));
        });
        it('should reject VENDOR with pending KYC', async () => {
            const vendor = createMockVendor({ kycStatus: client_1.KycStatus.PENDING });
            const password = 'Password123!@#';
            vendor.password = `$2a$10$hashed_${password}`;
            prisma.user.findUnique.mockResolvedValue(vendor);
            userValidationService.validateLoginEligibility.mockImplementation(() => {
            ...
            await expect(service.login({ email: vendor.email, password }))
                .rejects.toThrow(new common_1.HttpException('KYC verification required', common_1.HttpStatus.FORBIDDEN));
        });
        it('should reject VENDOR with rejected KYC', async () => {
            const vendor = createMockVendor({ kycStatus: client_1.KycStatus.REJECTED });
            const password = 'Password123!@#';
            vendor.password = `$2a$10$hashed_${password}`;
            prisma.user.findUnique.mockResolvedValue(vendor);
            userValidationService.validateLoginEligibility.mockImplementation(() => {
        ...
        it('should save refresh token after successful login', async () => {
            ...
            expect(tokenService.saveRefreshToken).toHaveBeenCalledWith(user.id, expect.any(String));
        });
    ...
    describe('resetPassword', () => {
        ...
        it('should reject non-existent user', async () => {
            ...
            await expect(service.resetPassword({
                ...
                token: validToken,
                newPassword: 'NewPassword123!@#',
            })).rejects.toThrow(new common_1.HttpException('Invalid or expired token', common_1.HttpStatus.BAD_REQUEST));
        });
        it('should reject invalid token', async () => {
            ...
            await expect(service.resetPassword({
                ...
                token: 'wrong-token',
                newPassword: 'NewPassword123!@#',
            })).rejects.toThrow(new common_1.HttpException('Invalid token', common_1.HttpStatus.BAD_REQUEST));
        });
        it('should reject expired token', async () => {
            ...
            await expect(service.resetPassword({
                ...
                token: validToken,
                newPassword: 'NewPassword123!@#',
            })).rejects.toThrow(new common_1.HttpException('Reset token has expired', common_1.HttpStatus.BAD_REQUEST));
        });
        it('should reject if no reset token exists', async () => {
            ...
            await expect(service.resetPassword({
                ...
                token: validToken,
                newPassword: 'NewPassword123!@#',
            })).rejects.toThrow(new common_1.HttpException('No active reset request', common_1.HttpStatus.BAD_REQUEST));
        });
    });
    describe('logout', () => {
        it('should successfully logout with valid token', async () => {
            ...
            expect(result.success).toBe(true);
            expect(tokenService.revokeRefreshToken).toHaveBeenCalledWith('user-id-123', 'valid-refresh-token');
        });
