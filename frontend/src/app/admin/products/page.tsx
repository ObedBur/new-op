...
const ProductRow: React.FC<{ product: AdminProduct; onClick: () => void }> = ({ product, onClick }) => {
    ...
    >
        ...
        <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-500 font-semibold truncate mt-0.5">{t('adminPage.by')} {product.seller}</p>
        </div>
...
const ALL_QUARTIERS = '__all__';
const QUARTIERS = [ALL_QUARTIERS, 'Virunga', 'Birere', 'Himbi', 'Katindo', 'Karisimbi'];
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="font-bold text-slate-700">{t('adminPage.productsLoadError')}</p>
        ...
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
            ...
            <div className="flex items-center justify-between gap-4">
                <div className="flex gap-2 overflow-x-auto no-scrollbar flex-1">
                    {QUARTIERS.map(q => (
                        <button
                            key={q === ALL_QUARTIERS ? t('adminPage.allMarkets') : q}
            ...
            {filtered.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-sm font-bold text-slate-500">{t('adminPage.noProducts')}</p>
                    <p className="text-xs text-slate-400 mt-1">{t('adminPage.changeMarketFilter')}</p>
                </div>
            )}
