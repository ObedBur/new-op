...
export const DesktopHeader = ({
  ...
  return (
    <div className="hidden lg:flex w-full h-24 items-center justify-between px-10 relative bg-transparent z-50">
      ...
      >
        {isAuthPage ? (
          <Link
            href="/"
            className="px-6 py-2 rounded-full text-xs font-bold tracking-widest[0.1em] uppercase bg-[#E67E22] text-white shadow-lg shadow-[#E67E22]/25 hover:scale-105 transition-all flex items-center gap-2"
          >
          ...
          <nav className="flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                ...
                href={link.id}
                className={`
                  px-5 py-2 rounded-full text-xs font-bold tracking-widest[0.1em] uppercase transition-all duration-300
                  ${isActive(link.id) 
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none group z-10">
        <Link href="/" className="flex flex-col items-center pointer-events-auto">
          <h1 className="text-4xl font-black tracking-[0.3em] drop-shadow-md group-hover:scale-105 transition-transform duration-500 uppercase">
            <span className="text-[#E67E22]">WAPI</span><span className="text-[#2D5A27]">BEI</span>
           </h1>
          <div className="w-12 h-1 bg-[#E67E22] rounded-full mt-1 opacity-0 group-hover:opacity-100 group-hover:w-20 transition-all duration-500"></div>
        </Link>
      </div>
